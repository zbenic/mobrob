...
class PIDControllerPosition(object):
    FD_ROLL = 0
    FD_PITCH = 1
    FD_YAW = 2
    ...
    def __init__(self,
        ...
        self.Kp = [pid_x[0], pid_y[0], pid_z[0], pid_roll[0], pid_pitch[0], pid_yaw[0]]
        self.Kp = [self.PTERM_SCALE * p for p in self.Kp]

        self.Ki = [pid_x[1], pid_y[1], pid_z[1], pid_roll[1], pid_pitch[1], pid_yaw[1]]
        self.Ki = [self.ITERM_SCALE * i for i in self.Ki]

        self.Kd = [pid_x[2], pid_y[2], pid_z[2], pid_roll[2], pid_pitch[2], pid_yaw[2]]
        self.Kd = [self.DTERM_SCALE * d for d in self.Kd]


        self.itermLimit = itermLimit 

        self.previousRateError = [0]*6
        self.previousTime = 0 
        self.previous_motor_values = [self.minthrottle]*4
        self.pid_xzy_rpy = [PID(*pid_x), PID(*pid_y), PID(*pid_z), PID(*pid_roll), PID(*pid_pitch), PID(*pid_yaw)]

    def calculate_motor_values(self, current_time, desiredVals, measuredVals):
        xyz_yaw_sums = []
        for i in range(6):
            self.pid_xzy_rpy[i].SetPoint = desiredVals[i]
            self.pid_xzy_rpy[i].update(current_time, measuredVals[i])
            if i == 0:
                desired_X_dot = self.pid_xzy_rpy[i].output
            elif i == 1:
                desired_Y_dot = self.pid_xzy_rpy[i].output
                desiredVals[3] = np.cos(measuredVals[5])*desired_Y_dot - np.sin(measuredVals[5]) * desired_X_dot
                desiredVals[4] = np.cos(measuredVals[5])*desired_X_dot + np.sin(measuredVals[5]) * desired_Y_dot
            elif i == 2:
                xyz_yaw_sums.append(self.pid_xzy_rpy[i].output + 9.81)
            else:
                xyz_yaw_sums.append(self.pid_xzy_rpy[i].output)
        return self.mix(*xyz_yaw_sums)
    ...
    def mix(self, z, r, p, y):
        ...
        scaledAxisPidZ = self.constrainf(z, -pidSumLimit, pidSumLimit) / 10.0
